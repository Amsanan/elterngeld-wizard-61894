...
serve(async (req) => {
  ...
  try {
    ...
    const { filePath, zeugnisTyp = 'et_bescheinigung', useLLM = true } = await req.json();
    
    if (!filePath) throw new Error('No file path provided');

    console.log('Processing Ärztliches Zeugnis file:', filePath, 'Type:', zeugnisTyp);
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        data: insertedData,
        message: 'Ärztliches Zeugnis erfolgreich extrahiert',
        ocrText: ocrText
      }),

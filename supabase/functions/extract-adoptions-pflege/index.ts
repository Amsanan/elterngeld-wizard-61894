  ...
  try {
    ...
    if (ocrResult.ParsedResults?.length > 0 && !ocrResult.IsErroredOnProcessing) {
      ...
      if (useLLM) {
        ...
        // Merge LLM results but preserve dokumenttyp from request
        const { dokumenttyp: _, dokument_typ: __, ...llmDataWithoutTypes } = llmResult.data;
      ...
      return new Response(
        JSON.stringify({
          success: true,
          data: insertedData,
          message: "Adoptions/Pflege-Dokument erfolgreich extrahiert",
          ocrText: ocrText,
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );

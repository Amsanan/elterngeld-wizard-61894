...
const ArbeitgeberbescheinigungList = () => {
      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={() => navigate("/upload-arbeitgeberbescheinigung")}>
              <Upload className="h-4 w-4 mr-2" />
              Neue hochladen
            </Button>
          </div>
          <h1 className="text-2xl font-bold text-foreground">
            Meine Arbeitgeberbescheinigungen ({documents.length})
          </h1>
        </div>
      </header>
        ...
        {documents.length === 0 ? (
          <Card className="max-w-2xl mx-auto p-12 text-center">
            <p className="text-muted-foreground mb-4">
              Noch keine Arbeitgeberbescheinigungen hochgeladen
            </p>
            <Button onClick={() => navigate("/upload-arbeitgeberbescheinigung")}>
              <Upload className="h-4 w-4 mr-2" />
              Erste Bescheinigung hochladen
            </Button>
          </Card>
        ) : (
                ...
                <div className="flex items-start justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium text-foreground">
                          {doc.arbeitgeber_name || "Nicht verfügbar"}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Bruttogehalt</p>
                        <p className="font-medium text-foreground">
                          {doc.bruttogehalt ? `${doc.bruttogehalt} €` : "Nicht verfügbar"}
                        </p>
                      </div>
                      <div>
                  ...
                  <div className="flex gap-2 ml-4">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        navigate(`/arbeitgeberbescheinigung-result?id=${doc.id}`)
                      }
                    >
...
export default ArbeitgeberbescheinigungList;
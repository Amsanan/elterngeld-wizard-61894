...
const DOKUMENT_LABELS: Record<string, string> = {
  ...
  hebammenzeugnis: "Hebammenzeugnis",
  ...
  einnahmen_ueberschuss_rechnung: "EÜR",
  krankentagegeld_bescheinigung: "Krankentagegeld",
  arbeitsvertrag: "Arbeitsvertrag",
  ausbildungsvertrag: "Ausbildungsvertrag",
  bezuegemitteilung_beamte: "Bezügemitteilung",
  tagespflege_eignung: "Tagespflege-Eignung",
  leistungsnachweis_ausland: "Leistungsnachweis Ausland",
  vertretungsnachweis: "Vertretungsnachweis",
  feststellungsbeschluss_familiengericht: "Feststellungsbeschluss",
  sonstige: "Sonstiges",
};

const SonstigeNachweiseList = () => {
      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={() => navigate("/upload-sonstige-nachweise")}>
              <Upload className="h-4 w-4 mr-2" />
              Neue hochladen
            </Button>
          </div>
          <h1 className="text-2xl font-bold text-foreground">
            Sonstige Nachweise ({documents.length})
          </h1>
        </div>
      </header>
        ...
        {documents.length === 0 ? (
          <Card className="max-w-2xl mx-auto p-12 text-center">
            <p className="text-muted-foreground mb-4">
              Noch keine sonstigen Nachweise hochgeladen
            </p>
            <Button onClick={() => navigate("/upload-sonstige-nachweise")}>
              <Upload className="h-4 w-4 mr-2" />
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="px-3 py-1 bg-primary/10 text-primary text-sm font-medium rounded-full">
                        {DOKUMENT_LABELS[doc.dokument_typ] || doc.dokument_typ}
                      </span>
                      {doc.person_type && (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium text-foreground">
                          {doc.aussteller || "Nicht verfügbar"}
                        </p>
                      </div>
                      {doc.betrag && (
                        <div>
                  ...
                  <div className="flex gap-2 ml-4">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        navigate(`/sonstige-nachweise-result?id=${doc.id}`)
                      }
                    >
...
export default SonstigeNachweiseList;

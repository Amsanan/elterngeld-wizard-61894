      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={() => navigate("/upload-bankverbindung")}>
              <Upload className="h-4 w-4 mr-2" />
              Neue hochladen
            </Button>
          </div>
          <h1 className="text-2xl font-bold text-foreground">
            Meine Bankverbindungen ({documents.length})
          </h1>
        </div>
      </header>
        ...
        {documents.length === 0 ? (
          <Card className="max-w-2xl mx-auto p-12 text-center">
            <p className="text-muted-foreground mb-4">
              Noch keine Bankverbindungen hochgeladen
            </p>
            <Button onClick={() => navigate("/upload-bankverbindung")}>
              <Upload className="h-4 w-4 mr-2" />
              Erste Bankverbindung hochladen
            </Button>
          </Card>
        ) : (
                ...
                <div className="flex items-start justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium text-foreground">
                          {doc.kontoinhaber || "Nicht verfügbar"}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Bank</p>
                        <p className="font-medium text-foreground">
                          {doc.bank_name || "Nicht verfügbar"}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">IBAN</p>
                        <p className="font-medium text-foreground font-mono text-sm">
                          {doc.iban || "Nicht verfügbar"}
                        </p>
                      </div>
                      <div>
                  ...
                  <div className="flex gap-2 ml-4">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        navigate(`/bankverbindung-result?id=${doc.id}`)
                      }
                    >
...
export default BankverbindungenList;
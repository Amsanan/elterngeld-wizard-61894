...
const UploadKindergeldBescheid = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const filePath = `${session.user.id}/kindergeld-bescheid/${fileName}`;
      ...
      toast({
        title: "Erfolg",
        description: "Kindergeld-Bescheid erfolgreich extrahiert!",
      });

      navigate(`/kindergeld-bescheid-result?id=${data.data.id}`);
    } catch (error: any) {
      toast({
        title: "Fehler",
      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-6">
          ...
          <h1 className="text-2xl font-bold text-foreground">
            Kindergeld-Bescheid hochladen
          </h1>
          <p className="text-muted-foreground mt-2">
            Laden Sie den Bescheid der Familienkasse über Kindergeld hoch.
          </p>
        </div>
      </header>
...
export default UploadKindergeldBescheid;
